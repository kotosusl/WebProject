...
async def add_response(update, context):  # добавление олимпиады по названию
    olimp = update.message.text  # название олимпиады
    session = db_session.create_session()
    result = session.query(Olimp).filter(Olimp.name.like(f'%{olimp}%')).all()  # олимпиады, совпадающие по названию
    if result:
        if len(result) <= 24:
            keys = [[InlineKeyboardButton(f'{j + 1}', callback_data=f'{result[j].id}*add') for j in range(i, i + 3)]
                    for i in range(0, len(result) - len(result) % 3, 3)]  # генерация кнопок с номерами
            if len(result) % 3 != 0:
            ...
            reply_keyboard = [['/help', '/start'],
                              ['/find', '/add'],
                              ['/unset', '/unset_all']]
            markup2 = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=False)  # панель с командами
            ...
            await update.message.reply_html('Для добавления олимпиады в список напоминаний выберите номер',
                                            reply_markup=markup2)  # вывод сообщений
        else:
            reply_keyboard = [['/help', '/start'],
                              ['/find', '/add'],
                              ['/unset', '/unset_all']]
            ...
            # вывод сообщения
            return ConversationHandler.END  # окончание диалога
    else:
        reply_keyboard = [['/help', '/start'],
                          ['/find', '/add'],
                          ['/unset', '/unset_all']]
        markup = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=False)  # панель с командами
        await update.message.reply_html("""Олимпиад не найдено""", reply_markup=markup)  # вывод сообщения
    return ConversationHandler.END  # окончание диалога
